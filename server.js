...
// Expanded English-to-Arabic word map
...
async function autoSearchQuran(userMessage) {
    ...
    const maxPages = isCountQuestion ? 6 : 2;
    ...
    if (searchWord) {
        ...
        // Arabic search for better coverage
        ...
        try {
            const response = await fetchWithTimeout(
                'https://generativelanguage.googleapis.com/v1beta/openai/chat/completions',
                {
                    ...
                    body: JSON.stringify({ model: 'gemini-2.0-flash', messages, temperature: 0.2, max_tokens: 1200 })
                }, 18000
            );
            if (response.ok) {
    ...
    try {
        ...
        if (!process.env.GEMINI_API_KEY && !process.env.OPENROUTER_API_KEY && !process.env.GROQ_API_KEY) {
            return res.status(500).json({
                error: 'AI not configured. Set at least one API key:\n' +
...
app.listen(PORT, () => {
    ...
    if (process.env.OPENROUTER_API_KEY) providers.push('OpenRouter (DeepSeek R1, Nemotron 550B, etc.)');
    if (process.env.GROQ_API_KEY) providers.push('Groq (Llama 3.3 70B)');
    
    console.log(`\n  ╔══════════════════════════════════════════╗`);
    console.log(`  ║  Baca AI Server v2 — port ${PORT}            ║`);
    console.log(`  ╠══════════════════════════════════════════╣`);
    console.log(`  ║  AI Providers:                          ║`);
    providers.forEach(p => console.log(`  ║  ✓ ${p.padEnd(38)}║`));
    if (providers.length === 0) console.log(`  ║  ✗ NO API KEYS CONFIGURED               ║`);
    console.log(`  ╚══════════════════════════════════════════╝\n`);
});
